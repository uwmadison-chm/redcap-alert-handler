...
* Global configuration contains:
  ...
  * dry_run (optional bool, default false; added 2026-07-22) -- the default for routes that don't set their own. A dry-run route dispatches its handler but the engine writes nothing to the message: no claim, no terminal state, no move (see step 7). Handlers are trusted to honor it by skipping their outward side effects; the engine can't stop a handler that ignores it.
  * durations in the config file accept humanfriendly strings (`"5s"`, `"3h"`) or bare TOML numbers meaning seconds
* Route-specific config is a mapping keyed on slug and containing at least a handler reference; `max_age` and `dry_run` may be set per-route, overriding the global default (so a dry-run instance can exempt one live route, or a live instance can dry-run one). Other keys are passed as part of the context to the handler, preserved opaquely per the brief. Global keys are included in the context; route-specific keys override. Handlers are responsible for handling their own validation and must ignore irrelevant context information.
...
* `checkup` (added 2026-07-24, after step 7, issue #8): an optional `checkup(context) -> list[str]` attached to the handler callable, which the engine calls with the route's ordinary `Context` and reports whatever comes back. This is how a route's config gets judged without the engine learning a single handler key -- the handler validates its own, and the diagnosis lands at doctor time instead of on every routed message. Attaching it to the callable rather than registering a second entry point keeps the check next to the handler it validates, and keeps `package:name` the only string an operator writes. A handler without one is normal, not a problem. A checkup that raises, isn't callable, or returns something other than strings is reported against its route; it can't take the check list down.
...
* Startup checks (added 2026-07-24, after step 7, issue #10): `rah process` runs the whole `rah doctor` check list before its first pass and reports every result at normal verbosity, so a broken config shouts once instead of message by message. The list moved out of `cli/doctor.py` into `checks.py` for this -- one list, one meaning per check, whichever command asked. Only `checks.REQUIRED_TO_START` (config, routes, handlers, secrets) stops the command. Everything else is advice: a failed graph, folders, or categories check falls through to the pass loop, which already exits nonzero in one-shot mode and rides the trouble out under `--watch`, and a failed checkup never stops anything -- the route processes mail and fails loudly per message, which is what the operator wants to see.
* dry_run (added 2026-07-22, after step 7): a dry-run route runs its handler but the engine writes nothing back -- `_claim_and_dispatch` skips the claim, and the outcome is logged without the terminal-state patch or the move, so the message is left exactly as found and re-runs every pass. That re-run-each-pass is by design; dry-run is a one-pass `rah process` inspection tool, not a watch mode. Whatever the handler raises (or returns) all resolves to the `dry_run` stat bucket. No `DryRun` exception exists: because the engine already knows the route is dry-run before it claims, the flag alone drives it and the handler just signals normally. All four decrement/retry-at/dead-letter concerns fall out of "never claimed, never written."
